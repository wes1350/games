...
export class BoardController {
    public static AddDomino(
        board: Board,
        domino: Domino,
        direction: Direction
    ): Board {
        const newBoard = _.cloneDeep(board);
        if (!BoardUtils.VerifyPlacement(board, domino, direction)) {
            throw new Error(
                `Domino ${DominoViewModel.TextRep(
                    domino
                )} cannot be added in the ${direction} direction`
            );
        }

        if (direction === Direction.NONE && !BoardViewModel.IsEmpty(newBoard)) {
            throw new Error(
                "Tried to add a domino without a direction when it was not the first domino"
            );
        }

        if (DominoViewModel.IsDouble(domino) && !newBoard.spinner) {
            ...
            if (!newBoard.spinner) {
                ...
                const arm = BoardUtils.GetArmByDirection(newBoard, direction);
